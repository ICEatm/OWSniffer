...
namespace OWSniffer
{
    class Sniffer
    {
        ...
        /// </summary>
        Regex ReplayServerMatcherWMSJ = new Regex(@"(replay\d+\.wmsj\.cn)");

        /// <summary>
        /// Matches the replay server packet data.
        /// </summary>
        Regex ReplayServerMatcherVALVE = new Regex(@"(replay\d+\.valve\.net)");
        ...
        private void HandlePacket(Packet packet)
        {
            ...
            {
                var matchWMSJ = ReplayServerMatcherWMSJ.Match(data);
                var matchVALVE = ReplayServerMatcherVALVE.Match(data);

                if (matchWMSJ.Success)
                {
                    ReplayServerAddr = matchWMSJ.Groups[1].Value;
                    Console.WriteLine("\n> Replay server: {0}", ReplayServerAddr);
                }

                if (matchVALVE.Success)
                {
                    ReplayServerAddr = matchVALVE.Groups[1].Value;
        ...
        public void Start()
        {
            ...
            Console.WriteLine("\n> Listening for Overwatch demo download...\n");
